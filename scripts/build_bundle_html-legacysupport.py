...
def inline_scripts(html: str, base_dir: Path) -> tuple[str, bool]:
    ...
    def repl(m):
        ...
        if src.startswith("js/"):
            src = src.replace("js/", "js-legacy/", 1)

        # libs/ → libs-legacy/
        if src.startswith("libs/") and src.endswith(".js"):
            src = src.replace("libs/", "libs-legacy/", 1)

        # Erityistapaus ZXing
        if src.endswith("libs/zxing-v0.19.1/index.min.js"):
            src = "libs/zxing-v0.19.1/index.legacy.js"
        # ------------------------------
