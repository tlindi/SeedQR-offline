...
window.getPayload = function (_ref) {
  var qr = _ref.qr,
    result = _ref.result;
  ...
  console.log('getPayload: source=' + src + ' rawLen=' + String(raw.length) + ' rawPreview=' + hexPreview(raw, 12));
  ...
  if (src === 'ZXing' && typeof window.stripNibblesAndSlice === 'function') {
    try {
      var nibbleStripped = window.stripNibblesAndSlice(raw, 3, 16); // strip 3 hex chars (12 bits)
      if (nibbleStripped && nibbleStripped.length === 16) {
        console.log('getPayload: used stripNibblesAndSlice(3) on ZXing raw — preview=' + bytesToHex(nibbleStripped));
  ...
  if (raw.length >= 19) {
    var headerStripped = raw.slice(3, 3 + 16);
    var headFirst = raw.slice(0, 16);
    ...
    var headFirstArr = Array.from(headFirst);
    var headerStrippedArr = Array.from(headerStripped);
    var headFirstZero = headFirstArr.every(b => b === 0);
    var headerStrippedZero = headerStrippedArr.every(b => b === 0);
